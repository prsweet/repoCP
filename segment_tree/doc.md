...
## Its best if the Segment Tree is the power of 2

- if n is not the power of 2, we make it to the next power, so keeping the  
size 4n is standard because:  
  - if n = 2:  
    tree_size_needed = 4 -> thats fine...
  - if n = 3:  
    tree_size_needed = 6 -> so we can take 8 as power of 2
  - if n = 5:  
    tree_size_needed = 10 -> we can do 16 which is more than 3n so to be
    safe we do 4n

## why we do in the power of 2

So that our code is free of if else for checking whether we exceed the
boundaries with just little more space
